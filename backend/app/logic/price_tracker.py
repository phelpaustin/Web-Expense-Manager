...
Trends are grouped by (item, unit), not item alone: price_per_unit means a
different thing per unit (price per kg vs. per litre vs. per count), so
mixing units for the same item name would compare unrelated numbers.
"""
...
def price_trends(expenses: list[dict]) -> list[dict]:
    """Price-change summary per (item, unit) pair, needs >= 2 purchases."""
    by_item: dict[tuple[str, str], list[dict]] = defaultdict(list)
    for e in expenses:
        desc = str(e.get("description") or "").strip()
        if desc:
            by_item[(desc, _unit(e))].append(e)

    results = []
    for (item, unit), rows in by_item.items():
        if len(rows) < 2:
            continue
...
def shop_comparison(expenses: list[dict], item: str, unit: str | None = None) -> list[dict]:
    """Compare average/min/max price of the same item (and unit) across shops."""
...
def price_history(expenses: list[dict], item: str, unit: str | None = None) -> list[dict]:
    """Timeline of (date, price, shop) points for a single item (and unit), oldest first."""
    item_norm = item.strip().lower()
    matches = [e for e in expenses if str(e.get("description", "")).strip().lower() == item_norm]
    if unit is not None:
        matches = [e for e in matches if _unit(e) == unit]
